...
namespace TwinRovaMageCC //0.2.6
{
   // This is a modified EmuMage for leveling as frost. Credit goes to Emu for the original script.
    public class TwinRovaMage : CustomClass
    {
        ...
        //mana gems
        private string[] manaGemNames = {"Mana Jade"};
        ...
        public override void Fight()
        {
            ...
            */
			...
			//health potion
			if (this.Player.HealthPercent <= 25 && this.Player.ItemCount("Greater Healing Potion") != 0)
			{
				this.Player.UseItem("Greater Healing Potion");
			}
					
            //wand
            if (this.Player.ManaPercent < 15 || Target.HealthPercent < 15)
            {
           ...
           // {
			...
			//}
             ...
             //      {
             //         this.Player.CastWait("Cone of Cold", 5000);
             //         this.Player.Cast("Frostbolt");
             //         return;
             //      }
             //   }
                //else if (this.Player.GetSpellRank("Cone of Cold") != 0 && this.Target.DistanceToPlayer <= 10)
                //{
                //   if (this.Player.CanUse("Cone of Cold") && this.Player.CanUse("Frostbolt"))
                //   {
                //      this.Player.CastWait("Cone of Cold", 4000);
                //      this.Player.Cast("Fostbolt");
                //      return;
                //   }   
                //}
            ...
            if (Target.GotDebuff("Frost Nova"))
            {
                ...
                {
                    bool res = Player.ForceBackup(6); //Set to 6 for CONE
                }
            }
            else
            {
